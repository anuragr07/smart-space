...
exports.userLogin = async function (req, res) {
    await User.findOne({ username: req.params.user }, function (err, user) {
        if (err) return res.status(500).send({ auth: false });
        if (!user) return res.status(404).send({ auth: false });

        var passwordIsValid = (req.params.pass == user.password)
        if (!passwordIsValid) return res.status(401).send({ auth: false});

        res.status(200).send({ auth: true, user_id: user.user_id, email: user.username});
    });
}
